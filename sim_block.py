...
(function(){
  ...
  function loadHist(api,cdn){
    return tryUrl(api,true).catch(function(){return tryUrl(cdn,false);}).then(function(d){
      // JSONL：可能被 GitHub API 包成 base64 文本，或 CDN 直接给文本
      var txt = (typeof d==='string')?d:(d&&d.content?b64ToObj(d.content):d);
