...
@media(max-width:640px){
  ...
  #qiuge-sim .sim-tbl-pos td:nth-child(9)::before{content:"收益率";font-weight:500;}
...
(function(){
  ...
  function buildPositions(state){
    ...
    return keys.map(function(code){
      ...
      // D1（2026-09-22 用户确认）：底仓(600036)成本按**摊薄成本**口径显示（= 券商 App「成本价」），
      //   卫星仓按买入均价。底仓有 5 轮"卖后买回"，买入均价被推高 30.00→39.11（+30.4%）展示失真；
      //   卫星仓只卖不买回，买入均价即真实买入价。**判定口径（盘中检测）不受影响**。
      var cost=((code==="600036"&&p.cost_diluted!=null)?p.cost_diluted:p.cost)||0,px=p.last_close||p.cost||0;
  ...
  function render(state,stats,history){
    ...
    var html='<div class="sim-kpis">'+kpis+'</div>'
      ...
      +'<div class="sim-tbl-wrap"><table class="score-tbl sim-tbl-pos"><thead><tr><th>代码</th><th>名称</th><th>成本价*</th><th>数量</th><th>最新收盘</th><th>当日涨跌</th><th>当日盈亏</th><th>总盈亏</th><th>收益率</th></tr></thead>'
      +'<tbody>'+buildPositions(state)+'</tbody></table></div>'
      +'<div class="sim-note" style="margin-top:6px;">* 底仓（招商银行）成本价按<b>摊薄成本</b>口径（= 券商 App「成本价」，已把历次卖出的实现盈亏摊回）；其余标的为<b>买入均价</b>。「收益率」列为<b>累计收益率</b>（含已实现，按下单投入计）。</div>'
