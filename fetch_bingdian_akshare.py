...
数据优先级（应你的要求）：**legu 源作为主力 → 东财兜底 → 都无法获取则"暂未获取"**。
  ...
  D3 跌停数量   : 板块差异化 + 剔除ST 必须逐股判定，legu 只给聚合跌停（不区分板块/不保证剔除ST），
                 故 D3 权威值固定用东财 stock_zh_a_spot_em 逐股；legu 聚合跌停仅作交叉参考（_src 标注）。
...
def fetch_spot_em():
    """全量A股实时 spot → D1(兜底计数) + D3(板块差异化·剔除ST 权威值)。
    失败返回 (None, False)。"""
    try:
...
def build_bingdian_inputs():
    ...
    legu, legu_ok = fetch_breadth_legu()                      # 广度主源
    spot, spot_ok = fetch_spot_em()                           # 逐股（D1兜底 + D3权威）

    # ---- D1 选源：legu 主 → 东财spot 兜底 ----
    if legu_ok:
