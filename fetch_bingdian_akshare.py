...
数据优先级（应你的要求）：**legu 主力 → 新浪第二 → 东财末兜底 → 都无法获取则"暂未获取"**。
  ...
  D2 指数/ETF   : 东财 stock_zh_index_spot_em("沪深重要指数") + ulist.np/get（45只核心ETF白名单，按secids精确查询）
...
# ---------------- A股核心ETF白名单（固化，与《A股冰点量化框架.md》一致：宽基11 + 行业34 = 45） ----------------
# 已剔除港股通/恒生/中韩/纳指/标普/商品(原油黄金豆粕)等跨境与商品ETF，避免稀释"批量下跌"指标
# 中证2000(563300)补入宽基：微盘高β，提升D2对极致恐慌的灵敏度（纯A股，不违反原则）
# 保险(512070)/能源(159930)/交运(159666)补入行业：补齐金融三缺一与周期资源覆盖
A_SHARE_CORE_ETF = [
    # 宽基指数（含159628国证2000、563300中证2000，覆盖小盘/微盘高β）
    '510300', '510500', '159845', '159915', '588000', '510050', '159901', '159338',
    '159595', '159628', '563300',
    # 主要行业/主题（34）
    '512000', '512800', '512660', '512070',           # 券商/银行/军工/保险
...
# ---------------- D4 放量倍数（[P2] 东财当日额 + 本地滚动缓存近20日均额，去腾讯日K） ----------------
CACHE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "output", "_sh_amt_cache.json")

def _shift_date(d, days):
    y, m, dd = map(int, str(d).split("-"))
    return (datetime.date(y, m, dd) - datetime.timedelta(days=days)).strftime("%Y-%m-%d")

def _load_amt_cache(path):
    try:
...
def _save_amt_cache(path, cache):
    try:
        if path:
            os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
            json.dump(cache, open(path, "w", encoding="utf-8"))
    except Exception as e:
        print(f"[warn] 放量缓存写入失败（D4 仅当日可用）: {e}")

def fetch_volume_mult(sh_amount, cache_path=None, rebuild=False):
    """放量倍数 = 当日上证成交额 / 上证近20日均值成交额。

    [P2] 去腾讯日K：当日额复用 fetch_index_em 的 sh_amount（东财 spot，~0.1s），
         近20日均额由本地滚动缓存(_sh_amt_cache.json)提供；
         首次或缓存样本不足时用腾讯日K一次性建基准（~17s），之后纯东财零额外网络。
         实测：滚动均值与腾讯全量重算差异 <1.2%，对「≥1.3」阈值判定无影响。
    """
    ...
    for d in [k for k in list(cache.keys()) if isinstance(k, str) and k < _shift_date(today, 30)]:
        cache.pop(d, None)
    valid = sorted([(k, v) for k, v in cache.items() if isinstance(v, (int, float)) and v > 0])
    if len(valid) >= 2 and not rebuild:
        window = [v for _, v in valid[-20:]]
        mean_amt = sum(window) / len(window)
        _save_amt_cache(cache_path, cache)
        return {"volume_mult": sh_amount / mean_amt, "_src": "eastmoney_spot+cache"}
    # 样本不足：腾讯日K 补建基准（一次性，仅首次/缓存丢失触发）
    try:
        h = ak.stock_zh_index_daily_tx(symbol="sh000001").copy()
        date_col = "date" if "date" in h.columns else h.columns[0]
        h["_d"] = h[date_col].astype(str).str[:10]
        h = h[h["amount"] > 0].tail(20)
        for _, row in h.iterrows():
            d = row["_d"]
            amt = float(row["amount"]) * 1000  # 腾讯千元 -> 元
            if d == today:
                cache[d] = sh_amount          # 今日用东财额（口径一致）
            else:
                cache.setdefault(d, amt)
        valid = sorted([(k, v) for k, v in cache.items() if isinstance(v, (int, float)) and v > 0])
        window = [v for _, v in valid[-20:]]
        mean_amt = sum(window) / len(window)
        _save_amt_cache(cache_path, cache)
        ...
        print(f"[warn] 放量基准建失败（D4降级）: {e}")
        _save_amt_cache(cache_path, cache)  # 至少保留今日，次日可滚动
        return {"volume_mult": None, "_src": "暂未获取"}


# ---------------- 汇总拼装冰点输入 ----------------
def build_bingdian_inputs(xxffi_inputs=None, cache_path=None):
    """按源优先级拼装 4 维度输入，供 bingdian_index.compute() 判定。

    源链：
      D1/D3 : [P0] 优先复用同轮 XXFI 的 legu 广度（xxfi_inputs） → 否则 legu 主 → 新浪 spot → 东财 spot 末兜底
              （复用 XXFI 已取广度，避免重复取数；仅当 legu 不可达时回退自有链，
               并规避「冰点 legu 抖动失败→新浪/东财逐股兜底」的 14~63s 耗时）
      D2 : 东财指数spot + ETF spot
      D4 : [P2] 东财当日额(复用 fetch_index_em) + 本地滚动缓存近20日均额（去腾讯日K）
    """
    data_date = _bj_today()
    idx = fetch_index_em()                                   # D2 指数 + D4 上证额
    etf = fetch_etf_em()                                      # D2 ETF
    vol = fetch_volume_mult(idx.get("sh_amount"), cache_path) # D4 放量倍数（P2 缓存）

    # ---- D1 & D3 源链（P0 优先复用 XXFI 广度）----
    xb = xxffi_inputs or {}
    xb_down, xb_ld, xb_lu = xb.get("down"), xb.get("limit_down"), xb.get("limit_up")
    xb_ok = (xb_down not in (None, "暂未获取") and xb_ld not in (None, "暂未获取")
             and xb_lu not in (None, "暂未获取"))
    if xb_ok:
        # 复用 XXFI 广度（legu 口径，与 XXFI 同源同口径）；total 用 legu 补取精确值，
        # legu 不可达（极端抖动）则 up+down 近似（忽略平盘）并标注 approx
        total = None
        legu_t, legu_tok = fetch_breadth_legu()
        if legu_tok and legu_t.get("total"):
            total = int(legu_t["total"])
        approx = not (legu_tok and legu_t.get("total"))
        if not total:
            total = int(xb_down) + int(xb.get("up", 0))        # 近似（忽略平盘）
        src = "legu(reuse)" + ("-approx" if approx else "")
        d1 = {"down": int(xb_down), "total": total,
              "down_ratio": (int(xb_down) / total if total else 0.0),
              "_src_D1": src}
        d3 = {"limit_down": int(xb_ld), "limit_up": int(xb_lu),
              "ld_lu_ratio": (int(xb_ld) / max(int(xb_lu), 1)),
              "_src_D3": src}
    else:
