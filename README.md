...
|---|---|---|---|---|
| **D1 下跌广度** | 下跌 ≥ 4000 且 下跌占比 ≥ 85% | `legu`（聚合家数） | 新浪 `stock_zh_a_spot`（板块差异化·剔除 ST） | 东财 `stock_zh_a_spot_em`（板块差异化·剔除 ST） |
| **D2 指数/ETF 跌幅** | 上证 ≤ -2.0% 且 创业板指 ≤ -2.5% 且 核心 ETF 跌幅≤-2.5% 占比 ≥ 60% | 东财 `stock_zh_index_spot_em` + `fund_etf_spot_em`（42 只核心 ETF 白名单） | — | — |
| **D3 跌停数量** | 跌停 ≥ 50 且 跌停/涨停 ≥ 3 | `legu`（聚合跌停） | 新浪 `stock_zh_a_spot`（板块差异化·剔除 ST） | 东财 `stock_zh_a_spot_em`（板块差异化·剔除 ST） |
| **D4 放量恐慌** | 放量倍数（当日上证成交额 / 上证近 20 日均）≥ 1.3 | 东财当日额 / 腾讯 `stock_zh_index_daily_tx` 近 20 日均额 | — | — |

- **源链**：D1/D3 遵循 `legu 主 → 新浪 spot 兜底 → 东财 spot 末兜底`（与 XXFI 广度 `legu → 新浪 spot` 思路一致，但冰点把新浪升为第二源、东财仅最末兜底，因东财逐股 spot ~1min/58 页，正常 CI 不触发）；D2/D4 由东财/腾讯负责（指数、ETF、成交额类，东财直连快）。
- **板块差异化跌停**：主板 ±10%（≤-9.5%）、创业板/科创板 ±20%（≤-19.5%）、北交所 ±30%（≤-29.5%）、**ST/*ST 不纳入跌停统计**（按代码前缀 + 名称识别）。
- **溯源标记**：维度命中源记入 `_src_D1/D2/D3/D4`，取值 `legu` / `sina_spot` / `eastmoney_spot` / `eastmoney_index` / `eastmoney_etf` / `tencent_daily`；任一维度取数失败 → 该维度标 `暂未获取`；全部失败 → 冰点整体 `暂未获取`。
...
| `fetch_bingdian_akshare.py` | 冰点 akshare 取数器（D1/D3: legu 主 → 新浪 spot → 东财 spot；D2: 东财指数/ETF；D4: 东财额/腾讯均额；含多源兜底） |
