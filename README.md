...
| **D2 指数/ETF 跌幅** | 上证 ≤ -2.0% 且 创业板指 ≤ -2.5% 且 核心 ETF 跌幅≤-2.5% 占比 ≥ 60% | 东财 `stock_zh_index_spot_em` + `ulist.np/get`（secids 精确查 42 只核心 ETF 白名单，~0.1s） | `fund_etf_spot_em` 全量（命中不足/异常时兜底） | — |
...
- **源链**：
  ...
  - **D2（东财快）**：指数 spot（`stock_zh_index_spot_em`，~0.1s）+ ETF spot（**[提速] 改用东财 `ulist.np/get` + secids 精确查询 42 只核心 ETF 白名单，~0.1s**，替代原 `fund_etf_spot_em` 全量分页 37 页 ~18s；命中 <40 或异常时回退 `fund_etf_spot_em` 全量兜底，源标记 `eastmoney_etf(secids)`）。
...
| `fetch_bingdian_akshare.py` | 冰点 akshare 取数器（[P0] D1/D3 优先复用同轮 XXFI 的 legu 广度 → 否则 legu→新浪→东财；D2: 东财指数 + `ulist.np/get`(secids) 精确查 42 只核心 ETF，替代全量分页；[P2] D4: 东财当日额 + 本地滚动缓存近20日均额，去腾讯日K；含多源兜底） |
