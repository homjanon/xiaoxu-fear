...
**近30日趋势柱图口径（2026-08-25 修复）**：图表的「近90日峰值」参考线与「底部阈值(峰值×50%)」红线，以及柱体绿/灰染色，**统一使用 `_turnover_cache.json` 滚动窗口的真·近90日峰值 `max90_total`**——与卡片右上角明细、`is_bottom_today` 判定完全同源同口径。此前 bug：`dip_trend_svg()` 用 `max(hist30)`（仅30日窗口内最大值）冒充"近90日峰值"，一旦真峰值日滑出 30 日窗口（如 2026-08-25：真峰值 3.74 万亿 vs 图表假峰值 2.96 万亿、阈值 1.48 vs 正确 1.87 万亿），图表与卡片右上角两处数值自相矛盾、且今日柱漏染绿。`peak90` 缺失时回退窗口内最大值（兼容旧数据）。
...
> - **① 自建 CORS 代理 `xx`（优先）**：实时、无缓存，推完即最新——替代原 jsdelivr CDN（有 12h 缓存，数据不新鲜）；
