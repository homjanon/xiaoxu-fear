...
| **② 执行时机** | 避开开盘数据空窗，待资金流向定稿后执行 | cron `"30 7 * * 1-5"` = **北京 15:30**（GitHub Actions ~1h 延迟，实际约 16:30 执行）；`check` 步骤再校验「上证指数日K末交易日 == 今天」双保险，未更新则 skip |
...
- **东方财富**（资金流 2 项）：散户净流入 / 主力—散户背离 → `stock_market_fund_flow`（push2delay 补丁）。
